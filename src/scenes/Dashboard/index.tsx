    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const supplierslist = suppliers.data.result ||[]
          ...
          const selectAllOption = { id: 0, name: 'Select All' ,code:''};
...
{!loading && <div>
      ...
      <Card style={{ backgroundColor:"#fafafa", fontSize: "12px" }}>
        ...
        <Col className="gutter-row" span={4} style={{ flex: '1', maxWidth: '250px' }}>
          ...
          <Col className="gutter-row" span={4} style={{ flex: '1', maxWidth: '240px' }}>
            <div style={{ textAlign: 'left' }}>
  ...
  <div onClick={(e) => e.stopPropagation()} style={{ cursor: 'pointer' ,textAlign: 'center'}}>
    <SettingsIcon />
          ...
          { title: 'Ageing', dataIndex: 'ageing', align: 'center' ,width:100},
          {
            ...
            width: 300,
            render: (_, record) => {
              // Function to split the supplementaryInvoiceNo string into an array
              const splitValues = (values:string) => values ? values.split(',').map(v => v.trim()) : [];
          
              // Split the supplementaryInvoiceNo string
              const supplementaryInvoiceNo = splitValues(record.supplementaryInvoiceNo);
          
              // Determine the number of rows to render
              const maxRows = supplementaryInvoiceNo.length;
          ...
          {
            title: 'Date',
            dataIndex: 'supplementaryinvoicedatestring',
            align: 'center',
            width: 200,
            render: (_, record) => {
              // Function to split the supplementaryInvoiceNo string into an array
              ...
              // Split the supplementaryInvoiceNo string
              const supplementaryinvoicedatestring = splitValues(record.supplementaryinvoicedatestring);
          
              // Determine the number of rows to render
              const maxRows = supplementaryinvoicedatestring.length;
              ...
              return (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  {Array.from({ length: maxRows }).map((_, index) => (
                    ...
                    >
                      {supplementaryinvoicedatestring[index]}
                    </div>
                  ))}
                </div>
              );
