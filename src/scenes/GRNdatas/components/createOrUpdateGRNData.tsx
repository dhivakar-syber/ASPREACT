...
export interface ICreateOrUpdateGRNdataProps {
  visible: boolean;
  modalType: string;
  onCreate: () => void;
  onCancel: () => void;
...
class CreateOrUpdateGRNdata extends React.Component<ICreateOrUpdateGRNdataProps> {
  state = {
    visibleLookup: false,
  ...
  async getAllPartForLookupTable() {
    ...
    this.props.grndataStore.getAllPartForLookupTable(this.pagedFilterAndSortedRequest); 
  }
  async getAllSupplierForLookupTable() {
    ...
    this.props.grndataStore.getAllSupplierForLookupTable(this.pagedFilterAndSortedRequest); 
  }
  async getAllBuyerForLookupTable() {
    ...
    this.props.grndataStore.getAllBuyerForLookupTable(this.pagedFilterAndSortedRequest); 
  }
  ...
  render() {
    ...
    const lookupData: GRNdataPartLookupTableDto[] = this.props.grndataStore.partlookupdata?.items || []; 
    const supplierData: GRNdataSupplierLookupTableDto[] = this.props.grndataStore.supplierlookupdata?.items || []; 
    const buyerData: GRNdataBuyerLookupTableDto[] = this.props.grndataStore.buyerlookupdata?.items || []; 
    ...
    return (
      <div>
        <Modal visible={visible} onCancel={onCancel} onOk={onCreate} okText={L('Save')} title={L('GRNDatas')} width={550}>
      ...
      <Input />
            ...
            <Form.Item label={L('Quantity')} name={'quantity'} {...formItemLayout}>
              <Input />
            </Form.Item>
...
export default CreateOrUpdateGRNdata;