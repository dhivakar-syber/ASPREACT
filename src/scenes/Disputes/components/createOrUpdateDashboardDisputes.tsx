...
class CreateOrUpdateDahBoarddisputedata extends React.Component<ICreateOrUpdateDahBoardDisputesDataProps> {
  ...
  async getAllsupplierrejectionForLookupTable() {
    ...
    try {
      const result = await this.props.disputesStrore.getAllsupplierrejectionForLookupTable(this.pagedFilterAndSortedRequest);
  
      // Assuming `result` has an `items` property that contains the array data
      this.rejectData = result.items || []; // Use an empty array if `items` is undefined
    } catch (error) {
      console.error('Error fetching supplier rejection data:', error);
    }
  }
  ...
  //  const rejectionData: DisputeSupplierRejectionLookupTableDto[] = rejectData?.items || []; 
    ...
    //   if(initialData){
    //     formRef.current?.setFieldsValue({
    //         supplierName: initialData.buyerName,
    //         buyerShortId: initialData.supplierName,
    //       });
    //   }
    }}>
      {/* <Form.Item label={L('Supplier')} name={'supplierName'} {...formItemLayout}>
            ...
            <Form.Item label={L('SupplierName')} name="supplierName" {...formItemLayout} >
              <Input  disabled value={initialData ? initialData.supplierName : ''}/>
            </Form.Item>
